...
export NAMESPACE="nginx-ingress-animals-demo"
...
# --- PREPARATION ---
cat files/template_https_ingress.yaml | sed -e "s/DNS_DOMAIN/$DOMAIN/g" -e "s/NAMESPACE/$NAMESPACE/g" > /tmp/https-ingress.yaml
...
execCmd "kubectl expose deployment bear --port=80 -n $NAMESPACE"
execCmd "kubectl expose deployment hare --port=80 -n $NAMESPACE"
execCmd "kubectl get svc,pods -n $NAMESPACE"

prtHead "Show the NGNIX Ingress Controller Setup"
execCmd "kubectl get ns"
execCmd "kubectl get svc -n nginx-ingress"
...
echo "     => curl https://animals.${DOMAIN}/bear"
echo "     => curl https://animals.${DOMAIN}/hare"
echo ""
echo "     # --- Domain Based Routing"
echo "     => curl https://bear.$DOMAIN"
echo "     => curl https://hare.$DOMAIN"
echo ""
