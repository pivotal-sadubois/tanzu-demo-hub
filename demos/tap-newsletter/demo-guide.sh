...
if [ "$1" == "guide" ]; then 
  ...
  if [ "$2" == "predeploy" ]; then
    ...
    echo ""
    echo "3.)  Deploy Newsletter Subscription Service"
    ...
    echo ""
    echo "4.)  Deploy Newsletter UI Service"
    echo "     => tdh context dev"
    echo "     => tanzu apps workload apply --file config/workload.yaml --namespace $TAP_DEVELOPER_NAMESPACE \\"
    echo "          --local-path $HOME/workspace/newsletter/newsletter-ui --update-strategy replace --yes --tail"
    echo ""
