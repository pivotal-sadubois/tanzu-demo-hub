...
prtHead "Enable Read/Write Access to Primary and Secondary Postgres Node"
prtText "NOTE: Any clients or applications that attempt to connect to an HA Postgres cluster must"
prtText "      implement a “connection retry” mechanism, which issues retries until there is a valid"
prtText "      response from the correct server type. The “connection retry” mechanism must be provided "
prtText "      by the client application."
echo "     -------------------------------------------------------------------------------------------------------------------------------------------------------"
slntCmd "export PGTARGETSESSIONATTRS=read-write"
echo "     -------------------------------------------------------------------------------------------------------------------------------------------------------"
echo
...
ret=1
while [ $ret -ne 0 ]; do
  PGPASSWORD=$dbpass psql -h $dbhost -p $dbport -d $dbname -U $dbuser -f sql/tdh_info.sql; ret=$?
done
echo "-------------------------------------------------------------------------------------------------------------------------------------------------------"
...
ret=1
while [ $ret -ne 0 ]; do
  PGPASSWORD=$dbpass psql -h $dbhost -p $dbport -d $dbname -U $dbuser; ret=$?
done
...
ret=1
while [ $ret -ne 0 ]; do
  kubectl -n $NAMESPACE exec -it $INSTANCE-0 -- bash -c "psql"; ret=$?
done
...
execCat "$HELM_VALUES"
slntCmd "helm repo add cetic https://cetic.github.io/helm-charts > /dev/null 2>&1"
slntCmd "helm repo update > /dev/null 2>&1"
